...
struct mgc_memory_page {
	...
	struct mgc_memory_page *next;
	bool in_use;
};

struct mgc_memory_index_page {
	struct mgc_memory_index_page *next;

	size_t num_pages;
	struct mgc_memory_page pages[];
};

struct mgc_memory {
	struct mgc_memory_page *free_list;
	size_t num_pages_alloced;

	size_t head_num_pages_alloced;
	struct mgc_memory_index_page *head_index_page;
...
void
paged_list_init(struct paged_list *, struct mgc_memory *, size_t element_size);
...
struct arena {
	struct mgc_memory *mem;
	struct mgc_memory_page *head_page;
